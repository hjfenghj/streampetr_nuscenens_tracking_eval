...
from ...motmetrics_uto import MOTAccumulator as MMOTAccumulator
from ...motmetrics_uto import utils as Mutils
...
def test_metricscontainer_1():
    ...
    summary = m.compute(MMOTAccumulator.new_event_dataframe(), metrics=['mul','add'], name='x')
...
def test_metricscontainer_autodep():
    ...
    summary = m.compute(MMOTAccumulator.new_event_dataframe(), metrics=['mul','add'])
    assert summary.columns.values.tolist() == ['mul','add']
    assert summary.iloc[0]['mul'] == -3.
    assert summary.iloc[0]['add'] == 3.
...
def test_correct_average():
    ...
    acc = MMOTAccumulator(auto_id=True)
...
def test_motchallenge_files():
    ...
    def compute_motchallenge(dname):
        df_gt = Mio.loadtxt(os.path.join(dname,'gt.txt'))
        df_test = Mio.loadtxt(os.path.join(dname,'test.txt'))
        return Mutils.compare_to_groundtruth(df_gt, df_test, 'iou', distth=0.5)
    ...
    mh = Mmetrics.create()
    summary = mh.compute_many(accs, metrics=Mmetrics.motchallenge_metrics, names=dnames, generate_overall=True)

    print()
    print(Mio.render_summary(summary, namemap=Mio.motchallenge_metric_names, formatters=mh.formatters))
