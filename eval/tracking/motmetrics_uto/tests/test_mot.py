...
def test_events():
    ...
    expect = MMOTAccumulator.new_event_dataframe()
...
def test_max_switch_time():
    acc = MMOTAccumulator(max_switch_time=1)
...
def test_auto_id():
    ...
    acc = MMOTAccumulator(auto_id=False)
    with pytest.raises(AssertionError):
        acc.update([1, 2, 3, 4], [], [])
...
def test_merge_dataframes():
    ...
    r, mappings = MMOTAccumulator.merge_event_dataframes([acc.events, acc.events], return_mappings=True)    
