...
class MOTAccumulator(object):
    ...
    def update(self, threshold, class_name, hl_large_dist ,hl_small_dist, save_idsw_info, save_guanlian_info, save_highlight_info, NUSC, oids,\
                hids, dists, frameid=None, scene_name = "no set scene_name",timestamp = "no set timestamp",\
                    idsw_dirty_info_file = "do not print idsw"):
        ...
        Match_memery = []
        high_infos = []   # 设置pred和gt的距离阈值,超过该距离时,pred高亮
        self.dirty_events = True
        ...
        scene_name = NUSC.get("scene",scene_name)["name"]
        ...
        if oids.size * hids.size > 0:    
            ...
            for i in range(oids.shape[0]):
                if not oids[i] in self.m:  # 如果match中存在该gt
                    continue

                hprev = self.m[oids[i]]                    
                j, = np.where(hids==hprev)  
                if j.shape[0] == 0:   # 上一帧与某gt关联的pred消失
                    continue
                j = j[0]

                if np.isfinite(dists[i,j]):
                    oids[i] = ma.masked  # 被使用过的id加上掩码
                    hids[j] = ma.masked  
            ...
            dists[oids.mask, :] = np.nan   # 将使用过的gt(对应行)和pred(对应列)在数组中改为nan
            dists[:, hids.mask] = np.nan   
        
            rids, cids = linear_sum_assignment(dists)   # 匈牙利匹配完成真值和预测的目标分配, 返回矩阵中的行列索引值
            ...
            for i, j in zip(rids, cids):                
                ...
                # 当前帧的真值目标A在历史匹配容器中(表示这个真值目标不是第一次出现), 且当前帧跟它匹配的预测目标ID与历史容器中的预测ID不同, 最后还要保证当前帧与上一次目标A出现的帧，帧数差别不超过设定阈值
                # 这种判断方式是为了避免将目标跟踪中的短暂丢失或者目标重新出现的情况误判为ID切换。通过设置帧数差的阈值，可以过滤掉短暂的目标丢失或者重新出现，从而更准确地评估跟踪器的性能。
                is_switch = o in self.m and self.m[o] != h and abs(frameid - self.last_occurrence[o]) <= self.max_switch_time
                ...
                #todo  2023.09.18  fenghaojun
                ## 记录了每一个置信度阈值下发生的ID跳变情况，然后在退出所有阈值分数的计算逻辑以后, 通过MOTP(也可以是MOTA)最大得到的最有阈值分数，提取出该阈值分数下的所有id跳变情况
                if save_idsw_info:
                    if cat == "SWITCH":
                        with open(idsw_dirty_info_file,'a') as fp:
                            if threshold is not None:
                                text = scene_name + " "*20 + str(timestamp) + " "*20 + h + " "*20 + str(threshold) + " "*20 + class_name    # h代表预测目标的ID, 也可以记录真值ID(o)
                                fp.write(text+'\n')

            if save_highlight_info:
                for i in range(len(self._events)):
                    # if self._events[i][0] == "MATCH" or self._events[i][0] == "SWITCH":
                    if self._events[i][0] == "MATCH":
                        if class_name in ["trailer","car", "bus","truck"] and self._events[i][3] >= hl_large_dist:
                            high_infos.append([self._events[i][1],self._events[i][2]])                                                    # self._event[i][2]代表预测目标ID，self._event[i][1]为真值ID
                        elif class_name in ["bicycle","motorcycle","pedestrian"] and self._events[i][3] >= hl_small_dist:
                            high_infos.append([self._events[i][1],self._events[i][2]])
                        else:
                            continue

            if save_guanlian_info:
                for k,v in self.m.items():
                    Match_memery.append([k,v])
        ...
        # 
        return frameid,Match_memery,high_infos
