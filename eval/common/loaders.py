...
from utils.geometry_utils import points_in_box
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox:
                ...
                import sys
                sys.path.append("...")
                from eval.tracking.utils import category_to_tracking_name
...
# 判断某ann在视野外还是视野内
def view_IO(nusc, view_name, F):
    if view_name == "ALL":
        return True
    
    flag = False
    sample_token = F.sample_token
    sample = nusc.get("sample",sample_token)   #image的token

    cam_sample = sample["data"][view_name]
    image_info = nusc.get('sample_data',cam_sample)
    calib_data = nusc.get('calibrated_sensor',image_info['calibrated_sensor_token'])
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        if maxdist == -1.0:
            eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
                                             box.ego_dist <= max_dist[box.__getattribute__(class_field)] and view_IO(nusc,view_name,box)]
        else:
            eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if
                                         mindist<= box.ego_dist <= maxdist and view_IO(nusc,view_name,box)]
